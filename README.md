...
- 正式首页采用 A 版画册封面：纯白底、靛蓝标题、三张完整墨色作品叠放；创作请求、详情链接和计数随作品同步。
- Skill 简介位于首屏与画廊之间，画廊章节导航在浏览时吸顶。轮播间隔 6.5 秒，悬停、键盘聚焦、离开视口或页面隐藏时暂停，支持减少动态效果设置。
- 首屏可见作品使用至少 800w 的图片保留排线细节；其余两张进入展示区域时升级清晰度。
- 点击画廊里的任意作品跳转到它的独立详情页，浏览位置由浏览器的前进后退自然管理。
- 详情页左栏图片钉在视口不随滚动位移，右栏文字独立滚动；「上一张 / 下一张」常驻在
  图片下方，不用滚到底就能用，也支持键盘 <kbd>←</kbd> <kbd>→</kbd>。
- 详情页标题下可直接复制完整提示词；独立的「创作要点」依据设定介绍构图、墨色与笔触，不修改原始提示词。
- 使用方法提供可展开的 Codex / Claude Code 安装目录、完整文件夹检查和调用示例。
...
│  ├─ styles.css, home.css, app.js # 基础样式、A 版首页与轮播
...
仓库不包含原始出图 PNG 或测试截图；preview-a.html / preview-b.html 保留两份设计对比预览。
...
1. `assets/fonts/noto-serif-sc-subset.woff2`（约 193 KB）——站内实际用到的 455 个字，
...
- **交互**：轮播能否切换与暂停、是否共 5 张且点位同步、同一时刻是否只展示 3 张、
  ...
  返回是否恢复画廊滚动位置、创作请求与链接是否同步、禁用 JavaScript 后首页作品是否仍可打开
